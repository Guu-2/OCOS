...
router.get('/', function (req, res) {
  ...
  .get('/staff', async function (req, res, next) {
    const partial = 'partials/staff_manager';
    const layout = 'layouts/admin';
    req.partial_path = partial
    req.layout_path = layout
    req.page_data = {
      liststaff: await userControllers.getliststaff(),
  ...
  .get('/product', async function (req, res, next) {
    ...
    req.layout_path = layout
    req.page_data = {
      listproduct: await productControllers.getlistproduct(),
      // feature: req.session.admin_feature,
    }
  ...
  .get('/product/:id', authentication, async function (req, res, next) {
  ...
  .get('/customer', async function (req, res, next) {
    const partial = 'partials/customer_manager';
    const layout = 'layouts/admin';
    req.partial_path = partial
    req.layout_path = layout
    req.page_data = {
      listcustomer: await customerControllers.getlistcustomer()
    }
    await userControllers.getpage(req , res, next);
  })
  .get('/customer/:id', async function (req, res, next) {
  ...
  .get('/order/:id', async function (req, res, next) {
    const partial = 'partials/order_detail';
    const layout = 'layouts/admin';
    req.partial_path = partial
    req.layout_path = layout
    req.page_data = {
      listproduct: await orderControllers.getProductOfOrderById(req.params.id),
    }
    console.log("ABC" + req.page_data.listproduct)
    await userControllers.getpage(req , res, next);
  })
